...
- 当前 GitHub Pages 版已内置双公网房间同步脚本，不再依赖公司内网，也不再依赖 WebRTC/P2P。
...
建议先打开控制页面，再打开左/中/右三个显示页面。每个页面左下角会显示“公网同步 已连接 · room: test01”，并显示 `HTTP` / `MQTT` 两个通道状态。
...
同一组设备的 `room` 必须完全一致。未填写 `room` 时会默认使用 `test01`。
...
- 确认每台电脑左下角都显示“公网同步 已连接”。
- 如果 `HTTP` 或 `MQTT` 任意一个显示“通”，就可以同步。
- 如果两个都显示“断”，说明这台电脑无法访问公网同步通道。
- 左下角的“发 0s / 收 0s”表示最近一次发送或接收。调整设置后，其他电脑应看到“收 0s / 收 1s”。
- 修改设置后等待 1-2 秒，再看其他电脑是否同步。
- 建议先打开控制页面，再打开左屏、中屏、右屏。
- 不要混用公司内网地址 `10.133.72.116:3366` 和 GitHub Pages 公网地址。

正式展示时如果不想显示左下角同步状态，可以在网址后加：
`&syncStatus=0`
